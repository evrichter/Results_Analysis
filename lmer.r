...
Precritical_A$SE <- sd(Precritical_A$logRT, na.rm = TRUE) / sqrt(nrow(Precritical_A))

new_row_residuals <- data.frame(Region = 'Pre-critical', Condition = "A", Residual = Residual_Precritical_A, SE = Precritical_A$SE)
residuals <- rbind(residuals, new_row_residuals)


# calculate standard error for logRT estimated
##
new_row_logRT_estimated <- data.frame(Region = 'Pre-critical', Condition = "A", Estimated_logRT = Precrit_A_logRT_estimated)
